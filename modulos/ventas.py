...
def crear_cliente_desde_venta(cursor, datos_venta):
    """Crea el cliente y su subtipo fiscal si se ingresan datos desde el POS."""
    ...
    try:
        with conexion.cursor() as cursor:
            ...
            for item in carrito:
                ...
                stock_actual = cursor.fetchone()["stock_actual"]
                if float(stock_actual) < float(item["cantidad"]):
...
def mostrar():
    ...
    if productos.empty:
        ...
        opciones = {
            f"{row.nombre} | Código: {row.codigo_barras or 'Sin código'} | Stock: {row.stock_actual} {row.unidad_medida} | ${row.precio_venta:.2f}": row
                ...
                st.session_state["carrito"].append(
                    {
                        ...
                        "total_linea": float(cantidad) * float(producto.precio_venta),
                    }
                )
    ...
    if not carrito:
        ...
        if c13.button("Registrar venta", type="primary"):
            ...
            try:
                ...
                id_venta = registrar_venta(carrito, datos, id_usuario)
                st.session_state["carrito"] = []
    ...
    ventas = consultar_df(
        """
        SELECT v.id_venta, v.fecha, COALESCE(c.nombre, 'Cliente mostrador') AS cliente,
